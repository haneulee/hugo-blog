...
it('should be defined',()=>{ expect(service).toBeDefined();}); 이 코드로 확인해볼 수 있다.
{{< /hl-text >}}
...
it('should success to create user', ...: createAccount를 호출해서 성공적으로 계정 만드는 것을 테스트 하는 것입니다.
...
userRepository.findOne.mockResolvedValueOnce(null); : user repository의 findOne값을 흉내낸 것입니다.
...
userRepository.save.mockResolvedValue(TEST_HOST);: findOne과 마찬가지로 resolve 값을 리턴해줍니다. 성공적으로 만들어진 경우이므로 db에 성공적으로 저장된 값을 리턴하는 repository의 save 메소드를 흉내낸 것입니다.
